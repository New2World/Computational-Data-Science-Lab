...
__global__ void bfs(LL totalNodes,
                    ...
                    bool thread){
    int index = getIndex();
    LL nodeCount = 0, nodeRecord = index + 1;
    int que_h, que_t;
    LL adjNode, node;
    LL nodeSum = 0;
    float randProb;
    ...
    while(nodeRecord < totalNodes){
        ...
        if(!que_enque(queue, que_h, que_t, nodeRecord, index));    // overflow
        ...
        if(atomicCAS(nodeSet + nodeRecord, 0, nodeCount) != 0);
        nodeRecord += THREAD;
        nodeSum += nodeCount;
    }
    stop = clock();
